  ...
  const isSubscribed =
    ...
    function TrialProgressBar({ startDate, endDate, trialEligibility }) {
      ...
      try {
        ...
        // Calculate days left using floor to match backend calculation
        const diffTime = Math.max(0, endDateTime - currentTime);
        const daysLeft = Math.floor(diffTime / (1000 * 60 * 60 * 24));
        
        // Calculate days elapsed since start
        const elapsedTime = Math.max(0, currentTime - startDateTime);
        const daysElapsed = Math.floor(elapsedTime / (1000 * 60 * 60 * 24));
        
        // Calculate total available trial days (partial or full 7 days)
        const totalTrialDays = Math.floor((endDateTime - startDateTime) / (1000 * 60 * 60 * 24));
        
        // Calculate progress percentage based on a standard 7-day trial
        // If it's a partial trial, we need to adjust the calculation
        const daysConsumed = 7 - totalTrialDays + daysElapsed;
        const percentage = Math.min(100, Math.max(0, (daysConsumed / 7) * 100));
        
        // Determine if this is a partial trial based on total duration
        const isPartialTrial = totalTrialDays < 7;
        ...
        return (
          <div className="mt-4">
            ...
            {/* Better explanation of what the progress bar represents */}
            <div className="mt-2 text-xs text-muted-foreground">
              <p>
                {isPartialTrial
                  ? `Resuming previous trial (${daysLeft} days remaining from your last subscription)`
                  : "Full trial period"}
              </p>
            </div>
          </div>
