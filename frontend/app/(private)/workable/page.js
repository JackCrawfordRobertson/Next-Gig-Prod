  ...
  useEffect(() => {
    async function fetchJobs() {
      ...
      const jobsData = querySnapshot.docs.map((doc) => ({
        ...doc.data(),
        has_applied: doc.data().has_applied ?? false, // Default to false
      }));
      ...
      jobsData.forEach((job) => {
        if (job.date_added) {
          ...
          const weekday = format(jobDate, "EEEE"); // Get full weekday name
  ...
  return (
    <div className="h-screen w-full flex p-8 gap-6">
      ...
      <div className="w-1/3 h-full">
        <JobColumn title="Workable Jobs" jobs={jobs} />
      </div>

      {/* ✅ Right: Job Postings Chart */}
      <div className="w-2/3 h-full bg-card p-6 rounded-lg shadow">
        <CardHeader className="flex justify-between items-left">
          <div>
            <CardTitle>Workable Job Listings Per Week</CardTitle>
        ...
        <CardContent>
          <ChartContainer config={chartConfig}>
            <ResponsiveContainer width="100%" height={500}> {/* ✅ Increase Chart Height */}
            ...
            {jobs.length > 0 ? (
              jobs.map((job, index) => (
                <Card key={index} className="p-4 space-y-2">
                  ...
                  <p className="text-sm">Added on {job.date_added}</p>
